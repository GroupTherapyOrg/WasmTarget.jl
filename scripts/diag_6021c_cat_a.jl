#!/usr/bin/env julia
# diag_6021c_catA.jl — investigate Category A: "expected externref, found (ref $type)"
using Pkg
...
function dump_around_hex(func, arg_types, error_hex)
    ...
    hits = findall(l -> startswith(strip(l), "0x" * string(target_dec, base=16) * " |") ||
                        startswith(strip(l), "0x" * lpad(string(target_dec, base=16), 4, '0') * " |"), dump_lines)
    println("Lines at 0x$error_hex: $hits")
    if !isempty(hits)
        hit = hits[1]
        for i in max(1,hit-25):min(length(dump_lines),hit+15)
            mark = i == hit ? ">>>" : "   "
...
# _builtin_nothrow [81]: "expected externref, found (ref $type)" at 0x10ae
line81 = data_lines[findfirst(l -> startswith(l, "81 |"), data_lines)]
entry81 = resolve_func_entry(line81)
println("=== [81] $(entry81.mod).$(entry81.name) ===")
dump_around_hex(entry81.func, entry81.arg_types, "10ae")
