#!/usr/bin/env julia
#
# PURE-700: IR Audit Script — Catalog All Patterns Core.Compiler.typeinf Needs
#
# Walks the full dependency graph of Core.Compiler.typeinf(NativeInterpreter, InferenceState)
...
#
# Output: scripts/typeinf_audit_results.json
#
# Usage: julia +1.12 --project=WasmTarget.jl WasmTarget.jl/scripts/audit_typeinf_patterns.jl
...
# ============================================================================
# IR Walking (same as audit_lowering_patterns.jl)
# ============================================================================
...
function detect_stubs!()
    ...
    with_logger(logger) do
        try
            bytes = WasmTarget.compile(Compiler.typeinf, (Compiler.NativeInterpreter, Compiler.InferenceState))
            println("Compiled typeinf: $(length(bytes)) bytes")
            ...
            try
                ...
                println("typeinf.wasm VALIDATES")
            catch e
                println("typeinf.wasm FAILS validation: $e")
            end
...
function generate_output()
    ...
    outpath = joinpath(@__DIR__, "typeinf_audit_results.json")
...
function main()
    println("=== PURE-700: IR Audit Script for Core.Compiler.typeinf ===")
    println("Auditing IR patterns for typeinf(NativeInterpreter, InferenceState)...")
    println()

    # Phase 1: Walk IR dependency graph via code_typed
    println("Phase 1: Walking IR dependency graph via code_typed...")
    println("  Entry: Core.Compiler.typeinf(NativeInterpreter, InferenceState)")
    walk_dependencies!(Compiler.typeinf, (Compiler.NativeInterpreter, Compiler.InferenceState))
