#!/usr/bin/env julia
# diag_errors33.jl — Get EXACT wasm-tools validate error for each failing function
...
for (name, func_getter, arg_types) in test_functions
    ...
    try
        ...
        # Validate and capture FULL stderr
        proc = Base.run(pipeline(`wasm-tools validate --features=gc $tmpf`, stdout=devnull, stderr=tmpf * ".err"); wait=false)
        wait(proc)
        if proc.exitcode == 0
            println("  VALIDATES ✓")
        else
            err = read(tmpf * ".err", String)
