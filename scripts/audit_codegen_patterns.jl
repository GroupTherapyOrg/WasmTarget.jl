#!/usr/bin/env julia
#
# PURE-800: IR Audit Script — Catalog All Patterns WasmTarget.compile Needs (Self-Hosting)
#
# Walks the full dependency graph of WasmTarget.compile(::Function, ::Type{Tuple{Int64}})
# using Base.code_typed, and catalogs every IR pattern the compiler encounters.
# Then compiles with WasmTarget to detect which patterns are stubbed.
#
# This is M_CODEGEN: the compiler compiling itself.
#
# Output: scripts/codegen_audit_results.json
#
# Usage: julia +1.12 --project=WasmTarget.jl WasmTarget.jl/scripts/audit_codegen_patterns.jl

using JSON, Logging, WasmTarget
...
function detect_stubs!()
    ...
    with_logger(logger) do
        try
            bytes = WasmTarget.compile(WasmTarget.compile, (Function, Type{Tuple{Int64}}))
            println("Compiled codegen: $(length(bytes)) bytes")

            # Write wasm and validate
            tmpf = tempname() * ".wasm"
            write(tmpf, bytes)
            try
                run(`wasm-tools validate --features=gc $tmpf`)
                println("codegen.wasm VALIDATES")
            catch e
                println("codegen.wasm FAILS validation: $e")
            end
            ...
            try
                ...
                println("codegen.wasm: $nfuncs funcs")
            catch; end
        catch e
...
function generate_output()
    result = Dict(
        "entry_point" => "WasmTarget.compile(Function, Type{Tuple{Int64}})",
    ...
    outpath = joinpath(@__DIR__, "codegen_audit_results.json")
...
function main()
    println("=== PURE-800: IR Audit Script for WasmTarget.compile (Self-Hosting) ===")
    println("Auditing IR patterns for compile(Function, Type{Tuple{Int64}})...")
    println()

    # Phase 1: Walk IR dependency graph via code_typed
    println("Phase 1: Walking IR dependency graph via code_typed...")
    println("  Entry: WasmTarget.compile(Function, Type{Tuple{Int64}})")
    walk_dependencies!(WasmTarget.compile, (Function, Type{Tuple{Int64}}))
