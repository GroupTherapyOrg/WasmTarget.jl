...
# TRUE-TI-001: Precached version for WASM execution.
# Takes a pre-computed Vector{Method} directly, bypassing _get_all_methods
# which calls methods(f).ms (requires C runtime, traps in WASM).
# The matching algorithm (subtype checks, sorting, pruning) is identical.
function wasm_matching_methods_precached(@nospecialize(sig), all_meths::Vector{Method}, limit::Int)::Union{MethodLookupResult, Nothing}
    isempty(all_meths) && return MethodLookupResult(Any[], WorldRange(UInt(1), typemax(UInt)), false)
