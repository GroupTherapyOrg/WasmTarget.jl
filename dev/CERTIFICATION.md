...
| Determinism | class ids are a function of the program (`ClassIdNumbering`, `class_info.dart:831`); no world ages | struct registry walked in one order; one `WASM_WORLD_AGE`; Dict constants serialize only occupied slots; byte identity witnessed cross-process | L112; `test/probe_bytes.jl` (235 probes) | ✅ |
