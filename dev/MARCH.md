...
| 13.12 | C9 | the repository clean outside `src/`: the src comments and message that cite deleted fuzz docs (diagnostics.jl `STRICT_MODE_INVENTORY.md`; invoke.jl, julia_numeric_tier.jl `FINDINGS.md`) rewritten; the two open fuzz gaps with no runnable reproducer (`1f6e77980994`, `8558c726082d`) made smoke cases or deleted; the apparatus self-checks (`test/fuzz/test_*.jl`), `stdlib_coverage.jl` and `dev/merge_baseline.py` wired into a lane or deleted; then C9's liveness check as a lock | C9's planned check replaced by a lock at 0 |
