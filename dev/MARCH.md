...
| 13.12 | C9 | C9's liveness check as a lock: every tracked file outside `src/` consumed by the build, a test, a lane, CI or the docs site (by path, or by the loader that walks its directory), and no ledger gap `status: fixed`; needs its lock ID cited in `dev/CHARTER.md` C9 | C9's planned check replaced by a lock at 0 |
