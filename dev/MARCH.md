...
| H | (2) (4) done; (new) runtime-Vararg splat done `7f64f7c1` (one resolver `_apply_iterate_vararg_target_mi` consulted by both the enroller and the pruner; `Tuple(v)`/`tuple(v...)`/`Core.tuple(v...)` matched by object identity — only the first used to lower; L40 restated; smoke +5 `varargs` group, full test green); (1) root cause found on `march/p12H-atsit5` (`256303d7`, UNGATED checkpoint: `is_nonoverlayed(interp)` is always false under WasmInterpreter → pure type machinery labelled :semi_concrete_eval; `_wt_reads_overlaid_method` re-derives the label — the agent's last measurement still reported `overlaid=true` for `_compute_eltype`, cause not yet located: either a source-availability gap in the walker or a genuine capability gap) |
...
| D | in flight — PAUSED 2026-09-08 with branches pushed | stage 1 `march/p12D1-nir` COMPLETE at `588b9d43` (9 commits; measured by its agent: R29a 425→244, R29b 142→118, statements.jl 179/21→2/0, values.jl 7/2→0/0, R30 619→615, R31 20→19; probes 225/0 and smoke 81/81 at every commit; full Pkg.test 2531/0; six locks re-pinned and negative-tested: L119 L90 L18 L83 L47 L79). Based on pre-rewrite ids: rebase `--onto` the tip from `b09f3993`. stage 2 `march/p12D2-analyses` (commits `d775a8bf` `1f8e2c29` `0330b31e` over NIR: dispatch.jl 6→0, box_capture.jl 48→0, trimcollect.jl 29→0) lands after D1 with `9eaded62` DROPPED (D1's `588b9d43` carries the same NirInvoke.callee/`nir_retarget_invoke!` content) and every NIR `node.args` read renamed to `node.operands` (D1's `0bd1fe6a` rename); re-measure R29a after the rebase. stage 3 (calls/invoke/compile; delete `nir_operand`/`_ctx_ir`/`NirStmt.raw`/the `code_info` field) not started |
| K | open | |
