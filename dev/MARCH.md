...
### Phase 12 — The finish: the task list (written 2026-09-02, the march's remaining work)

The perfect ending, measured against §0: every ratchet that names a parallel path or a
re-derivation is **0 and locked**; every algorithmic component has a model; every failure is
located (done); one inference path; nothing enumerated where a rule exists; the capability gaps
found on the way closed with reproducers; and PR #122 merged with the release notes that say
exactly what changed. Each item carries its exit check. Order is by dependency, then value.

| # | Task | Why (the misdirect it removes) | Exit check |
|---|---|---|---|
| A | **One inference path.** `get_typed_ir`'s default `interp=nothing` runs Julia's native interpreter; the closed-world plan runs `WasmInterpreter`. The two produce different IR for the same function (measured tonight: the native one hid the `sizeof(String)` branch). Default = `WasmInterpreter`; delete the native path; lock. | two IRs for one function is a parallel path; it misled a root-cause for 20 minutes | `get_typed_ir` has one interpreter; L120 forbids `code_typed`/`get_typed_ir` without it in src; probes/smoke unchanged |
| B | **Number the closed world once** (dart `ClassIdNumbering`, class_info.dart:831). `_collect_reachable_ir_types` admits every concrete kind (primitives, `Char`, `Core.Box`, closures, `Memory`); delete `ensure_type_id!`'s allocating branch, `type_extra_ids`, and the extras-reading arms; `MCClassIdDispatch*` at `MaxLate = 0`. | the lazy numbering path is a second numbering; it produced the `isa` finding and the confinement lock (which becomes unnecessary) | `type_extra_ids` gone; `lazy_classid_isa.jl`'s confinement test replaced by "the lazy path does not exist"; ClassIdDispatch instances green at MaxLate 0 |
| C | **Constant evaluation by rule, not by list.** Replace `_is_typelevel_foldable`'s enumeration with the rule Julia itself uses — fold when effects are foldable **and nothrow** and the result is a program value (never a `Ptr`, never `objectid`/`hash` of a type) and the method is not overlaid — measured by the probe corpus and the family sweeps (the first attempt folded a layout read into a host address; the corpus caught it). | every new gap of the `isbitstype(String)` class otherwise needs another name added to a list | the enumeration is deleted; the rule is one predicate with a comment per exclusion; probes recorded; eighteen-family sweep green on both Julias |
| D | **The NIR migration to zero** (R29a 427 → 0, R29b 145 → 0). Convert context.jl (split `allocate_ssa_locals!` into join computation vs allocation, per §10.1), statements.jl, calls.jl, invoke.jl, compile.jl; `emit_value!` takes `NirNode`; delete `nir_operand`; `NirStmt.line` from DebugInfo so provenance lives in NIR and the diagnostics read it there (delete `_ctx_ir`). | raw `Expr.args[k]` reads are where the next silent misinterpretation of IR will come from; dart reads every node's type through one `typeContext` | R29a = R29b = 0 → locks; `code_info` appears only in ir.jl/nir.jl; probes byte-identical at every step |
| E | **The typed value channel** (Loop C, the largest). `compile_value`/`emit_value!` return the emitted `WasmValType`; every consumer reads it — no `infer_value_type` re-guess (R3 93 → 0), no `get_concrete_wasm_type` re-derivation at emit sites (R5 81 → 0); every coercion through `convert_type!` (R7 57 → 0, R27 54 → 0); typed `Box{T}` cells for captures (F3 L2) so `add_sum(Any, Any)`/boxed arithmetic lowers instead of rejecting. Model the funnel (`Coercion.tla`: dart `convertType` translator.dart:1597 — every (from, to) pair either converts exactly or rejects; no silent widening). | the re-guess sites are the only place a value's type can be computed twice differently; it is the mechanism behind every "boxed arithmetic" reject | R3 = R5 = R7 = R27 = 0 → locks; `sum(v::AbstractVector)` and the F3 counters compile; `Coercion.tla` in the harness with a Broken instance |
| F | **The last fifteen call arms → 0.** getfield/getproperty/setfield!/setproperty!/ifelse/typeassert/egal/isa/`+ - *` become self-contained registry entries that push their own operands; L38/L57 re-pinned to the entries; L116's allowlist deleted (the lock says 0). | an allowlist with reasons is honest, but it is still fifteen places a wrong edit can hide | R19 = 0 → lock; probes byte-identical; the negative controls unchanged |
| G | **Models for the remaining algorithmic components**: the closure layouter (dart `ClosureLayouter`, closures.dart:41-118), constant interning (`ensureConstant` canonicalization: one global per value, eager/lazy), the box-capture join (F3: the join of all writes), the NIR builder (total: every head classified or `NirUnsupported`), and after E the coercion funnel. A nightly workflow runs the deep instances (Stackifier N=5, ClassIdDispatch larger N) that are not CI-tractable. | these are the last components whose claims are only tested, never exhausted | each model paired + anchored (L111), Broken rejected; `formal-nightly.yml` |
| H | **Capability gaps with reproducers**, each: reproduce → located report → fix → smoke case. (1) SimpleATsit5: WT's inference widens an `apply_type(Pairs,…)` argument in DiffEqBase's kwcall glue that vanilla Julia resolves to `Const` (p8's finding); (2) `==(Any, String)` dynamic dispatch; (3) `Threads.Atomic` — `:invoke_modify` lowers to get/apply/set in a single-threaded module; (4) the WT-internal `str_*`/`arr_*` intrinsics: measure their users — delete them if nothing user-facing reaches them, else fix `generate_intrinsic_body`'s i32 assumption and the `Vector` boundary trap (p52's findings); (5) `sum(::AbstractVector)` on `Any` — falls out of E. | each is a loud reject today; the loop is now fast enough to close them one by one | smoke case per gap; capability negative controls updated where a reject becomes a lowering |
| I | **Strictness ratchets.** R30: codegen functions without a return-type annotation → 0; R31: `Any`-typed fields outside the named heterogeneous seams (`WasmDiagnostic.detail`, the registries' `Function` values, `NirLiteral.value`) → 0; the `@nospecialize` audit; ExplicitImports' `all_qualified_accesses_are_public` re-examined per module. | "strict in every regard" is not yet machine-checked for API types | R30 = R31 = 0 → locks |
| J | **The inner loop as one command.** `dev/lanes.sh` runs ratchet → smoke → probes → TLC (~1 min) and prints one verdict; `WT_PHASE` families and the `julia +1.13` / `+1.12.7~x64` legs documented in `dev/MARCH.md` §5. | every landing tonight typed the same five commands | `dev/lanes.sh` exit code is the commit gate; documented |
| K | **Close and release.** `dev/MARCH.md` final numbers, `dev/PARITY_MASTER.md` stamp on the final head's green run, `dev/CERTIFICATION.md` rows, `dev/HISTORY.md` entry; PR #122 merged; release 0.6.0 with notes (additive: located diagnostics, `WasmInternalError`; behavioral: `hash(::String)` now Julia's own, byte-identical builds across architectures). | the march is the record | main green; tag; JuliaRegistrator |

Not in the list, by decision: a rewrite of anything; JuliaLowering (experimental, 1.13-DEV,
...
| H | (2) (4) done; (new) runtime-Vararg splat READY on `march/p12H-apply-vararg` (rebased, smoke 87/87, probes 0 changed, full test was running at pause); (1) root cause found on `march/p12H-atsit5` (`256303d7`, UNGATED checkpoint: `is_nonoverlayed(interp)` is always false under WasmInterpreter → pure type machinery labelled :semi_concrete_eval; `_wt_reads_overlaid_method` re-derives the label) | `ca874e0c` — the WT-only `str_*`/`arr_*` surface deleted (1929 lines, L123); rethrow kept as ONE Method-keyed standalone body. `4702f60e` — (2): dispatch rows for boxed numerics and classed strings (the switch trapped with no row); singleton-function parameters resolve as callees. Open: (1) SimpleATsit5 widening, (3) `:invoke_modify`, (5) `sum(Any[…])` / `sum(f, ::Vector{AbstractVector})` = a dynamic call with TWO abstract positions (`add_sum(::Any, ::Any)`) — a product over observed classes was tried and blew up the closed world (10+ min on a 3-element program); needs a cascade design (dispatch.jl's selector cascades), not a product; NEW: `_apply_iterate` over a runtime Vararg tuple into a vararg callee (sparse `hvcat_internal` when not inlined); an erased multi-method callable (`Any[Adder(n)][1](x)`) |
...
| D | in flight — PAUSED 2026-09-08 with branches pushed | stage 1 `march/p12D1-nir` (9 commits: ctx-free `build_nir` first; `emit_value!(NirNode)`; `compile_statement!`/`compile_new!`/the foreigncall family/the partial-%new proof/`jl_type_intersection` on nodes; NirInvoke carries its callee) — R29a 425→~380; based on pre-rewrite ids (rebase `--onto` the tip from `b09f3993`). stage 2 `march/p12D2-analyses` (4 commits off D1's `979cfa61`: dispatch.jl, box_capture.jl, trimcollect.jl over NIR + a NirInvoke field) — lands after D1. stage 3 (calls/invoke/compile; delete `nir_operand`/`_ctx_ir`/`NirStmt.raw`/the `code_info` field) not started |
| K | open | |
