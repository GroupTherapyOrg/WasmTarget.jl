...
(*       statement IN THE SAME function body (`_f3_box_captors`, :93-104 --  *)
(*       scans `code`, not any closure's own body), retrieve each one's     *)
(*       typed IR through its `:invoke`'s CodeInstance/MethodInstance       *)
(*       (`_f3_capturing_closure_bodies`, :106-128), and fold in every      *)
...
(*       type`, :73-87). THIS STEP DOES NOT RECURSE: `_f3_capturing_        *)
(*       closure_bodies` is called EXACTLY ONCE, on the box's own function  *)
(*       (box_contents_type:155), and never again on a discovered closure's *)
(*       `ccode` to find a FURTHER-nested closure that ALSO captures the    *)
(*       same box object (passed one level further in, as `getfield(#self#,*)
(*       boxfield)` inside that closure's own body feeding a THIRD `:new`). *)
(*       A write living in such a grand-child closure is therefore          *)
(*       structurally INVISIBLE to this scan -- see the FINDING below.      *)
...
(*                                                                          *)
(* THE FINDING (item (2) above; reported alongside this model). Because the *)
(* closure-discovery scan is exactly ONE HOP from the box's own function,   *)
(* a write made by a closure NESTED INSIDE another captor closure (created  *)
(* by THAT closure's own `:new`, not the box function's) never enters the   *)
(* join. If the box's own function writes a concrete type (e.g. `x = 0`,    *)
(* Int64) and the one-hop child closure never writes directly (only calls   *)
(* the grandchild), `box_contents_type` returns Int64 even when the         *)
(* grandchild closure genuinely writes a Float64 -- a Wasm cell would be    *)
(* laid out as a narrow `Box{i64}` while a real write needs to store a      *)
(* Float64 into it, exactly the "silent mutable-capture 0" bug class this   *)
(* file's `dev/HISTORY.md#closures-and-dynamic-dispatch` section is named   *)
(* for. CONFIRMED by direct evaluation of `box_contents_type` on real typed *)
(* IR for                                                                  *)
...
(*     level1(n); x end                                                   *)
(* which returns `Int64` (WRONG -- should widen to `nothing`/anyref)        *)
(* although `x` is genuinely written both Int64 (init) and Float64          *)
(* (level2). Compiling this exact program end-to-end currently fails LOUDLY *)
(* with `:unsupported_method` on `level1`'s own invoke (a SEPARATE, older   *)
(* gap: WT's closed-world/invoke dispatch does not yet resolve a closure    *)
(* that itself creates and invokes a further closure) -- so today this      *)
(* specific join bug is not YET reachable as an observable silent-wrong-    *)
(* value miscompile; it would become one the moment that separate gap is    *)
(* fixed, and this model exists so it is caught by TLC first rather than    *)
(* found again the hard way. See the run report for the exact repro.        *)
(*                                                                          *)
...
(* never lets a write site outside its own one-hop discovery narrow the     *)
(* cell (an invisible write only ever costs precision -- forcing anyref --  *)
(* never soundness) PROVIDED discovery is transitive; §(2) above is the     *)
(* documented, currently-one-hop gap against that provision.                *)
(***************************************************************************)
...
CONSTANTS
    ...
    TransitiveDiscovery,  \* BOOLEAN: TRUE = closure writes discovered at ANY depth (the target
                          \* shape dart-parity needs); FALSE = today's real box_capture.jl (ONE hop
                          \* only -- _f3_capturing_closure_bodies never recurses, see the header)
