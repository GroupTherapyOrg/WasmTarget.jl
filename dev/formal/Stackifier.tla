...
(*                                                                          *)
(* INPUT NORMALIZATION (outside this model): the CFG the algorithm receives  *)
(* has first had its backward goto TRAMPOLINES threaded                     *)
(* (_thread_backward_trampolines!): a one-statement `goto` block whose      *)
(* target precedes it in linear order but does not dominate it is a forward *)
(* diamond laid out backwards by Julia's optimizer, not a loop; each         *)
(* predecessor is retargeted and the phi edges re-keyed, so the block is    *)
(* dead by reachability. The model's Init CFGs are therefore ones where     *)
(* every backward edge is a natural loop's (its target dominates its        *)
(* source); a dead block contributes no edge.                                *)
(*                                                                          *)
