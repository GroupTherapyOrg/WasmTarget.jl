...
## ✅ CRITICAL FINDING — FIXED (Loop-1 execution, 2026-06-28): multivar if/else phi-merge miscompile
...
to catch. An `if/else` whose branches assign **≥2 variables still live after the merge** kept only
ONE (the value routed through the `if (result T)` block); the others silently read 0.
`g1(7)`→8 (not 7008); `g3` kept only the last of 3; `sort2`/`twoUse` dropped one. Single-live-var
and ternary forms were correct (why the 2679-test suite was green). Confirmed native-vs-Node + WAT.
**ROOT FIX (Dale's root-cause mandate — commit pending):** route ANY merge with ≥2 phi nodes to
...
`generate_if_then_else`) that carried only one. Closed BOTH dispatch paths: `generate_complex_flow`
(stackified.jl `n_phi_nodes >= 2`) + `is_simple_conditional` (flow.jl). Full `Pkg.test()` + diff
fuzzer GREEN; regression guard CI-wired (`test/fuzz/repro_multivar_phi_merge.jl`). **NOT** what
`fix_consecutive_local_sets` did (the dropped store was never emitted — that pass is still genuinely
dead). **Impact on Loop 1:** the behavioral premise ("migrated emitters are correct, so the passes
are dead") held for the 7 fix_* passes (suite green under WT_NEUTRALIZE=all), and the precise
backfills additionally caught this INDEPENDENT flow-merge bug — now root-fixed. The mechanical fix_*
deletions resume on a now-correct foundation. (This is the ROOT-CAUSE MANDATE in action — see
`dev/CLEANUP_LOOP.md`.)
