...
| Determinism | Binaries are a function of the program: every identity-keyed registry is walked in program order, type objects carry a program-derived hash, the module has one world age (`WASM_WORLD_AGE`), and the probe corpus (`test/probe_bytes.jl`) witnesses byte identity across processes and architectures (x64 = aarch64) | L112, L117, L81/L31 |
