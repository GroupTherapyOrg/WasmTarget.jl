...
kind: compile_error
construct: "compile_error: `sort(push!(v, length(Dict(0 => 0, 0 => 0))))` :: Vector{Int64}→Vector{Int64}"
...
# Gap `61677a18a986` — compile_error: `sort(push!(v, length(Dict(0 => 0, 0 => 0))))` :: Vector{Int64}→Vector{Int64}
...
repro(v::Vector{Int64}) = sort(push!(v, length(Dict(0 => 0, 0 => 0))))
function _m(a, b)
...
Caused by:
    0: type mismatch: expected i64, found (ref $type) (at offset 0x562)
```
