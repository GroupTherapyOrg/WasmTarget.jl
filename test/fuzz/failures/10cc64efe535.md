...
Implemented per the fix design: new `generate_catch_arm_skip_merge`
generator + dispatch detection in the cas section (GotoNode skipping
the inner region with the paired GotoIfNot targeting the inner-try
side, arms merging at a phi whose edges are exactly {skip GotoNode,
inner catch tail}). Layout: outer try_table (returns), $armmerge block
wrapping [arm prefix; $skiparm cond split with skip-edge phi store +
br; inner try_table (+ optional body-edge merge store); inner catch
tail; catch-edge phi store], post-merge compiled after. e1cc-class
stub-flag resets before the condition and after every stackified
subset. Verified all inputs both versions; gates green.
