...
kind: wrong_value
construct: "wrong_value: `ceil(hypot(x, x))` :: Float64"
location: "test/fuzz (generated)"
fn_name: repro
arg_types: "(Float64,)"
first_seen: sweep-7-d4
---

# Gap `123e6c529ce7` — wrong_value: `ceil(hypot(x, x))` :: Float64
...
repro(x::Float64) = ceil(hypot(x, x))
...
## Diagnostic
```
at x=1.0e-300: native=1.0  wasm=NaN
```
