...
kind: compile_error
construct: "compile_error: `x + Float64(length(Dict(\"\" => 0, \"\" => 0)))` :: Float64"
...
# Gap `b32f12d94c73` — compile_error: `x + Float64(length(Dict("" => 0, "" => 0)))` :: Float64
...
```julia
using WasmTarget
repro(x::Float64) = x + Float64(length(Dict("" => 0, "" => 0)))
...
Caused by:
    0: type mismatch: expected i64, found (ref null $type) (at offset 0x2df)
```
