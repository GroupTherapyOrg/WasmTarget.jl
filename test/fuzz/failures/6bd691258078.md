...
kind: runtime_trap
construct: "runtime_trap: `map((y->getindex(Dict(\"\" => 0, \"\" => y), \"\")), v)` :: Vector{Int64}→Vector{Int64}"
...
# Gap `6bd691258078` — runtime_trap: `map((y->getindex(Dict("" => 0, "" => y), "")), v)` :: Vector{Int64}→Vector{Int64}
...
repro(v::Vector{Int64}) = map((y->getindex(Dict("" => 0, "" => y), "")), v)
_x = [0]
_c = deepcopy(_x)
