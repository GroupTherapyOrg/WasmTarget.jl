...
# Gap `f3829eae0fbb` — runtime_trap: `map((y->length(Dict(y => "", y => ""))), v)` :: Vector{Int64}→Vector{Int64}
...
repro(v::Vector{Int64}) = map((y->length(Dict(y => "", y => ""))), v)
function _m(a, b)
...
## Analysis
**Root cause: a `Dict` constructed/used inside a `map` kernel traps.** Same shape and
root as [[a771b83dda7c]] — `map(y->length(Dict(y=>"", y=>"")), v)` builds a Dict per
element inside the map closure; the compiled kernel mis-executes as a map-kernel
dependency (cf. [[3b005c4957f7]], [[5511171c8055]]). Here the Dict has String values
and the key is the closure variable `y`. **Deferred to Part 2** (deep codegen:
map-kernel-dependency + Dict-in-kernel).
