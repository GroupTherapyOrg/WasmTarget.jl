...
kind: runtime_trap
construct: "runtime_trap: `gcd(x, argmax(sort([0, 0, 1])))` :: Int64"
...
# Gap `c7d98df6cc56` — runtime_trap: `gcd(x, argmax(sort([0, 0, 1])))` :: Int64
...
repro(x::Int64) = gcd(x, argmax(sort([0, 0, 1])))
...
## Analysis
**FIXED by prior work** — recorded during the discovery sweep but already closed
by the committed `argmax`/`maximum`-signedness + narrow-shift fixes; `verify`
auto-closed it on first re-run with no new change. `argmax(sort([0,0,1]))`=3,
`gcd(0,3)`=3 now matches native.
