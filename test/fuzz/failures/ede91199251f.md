...
kind: wrong_value
construct: "wrong_value: `v` :: Vector{Float64}→Vector{Float64}"
...
# Gap `ede91199251f` — wrong_value: `v` :: Vector{Float64}→Vector{Float64}
...
## Diagnostic
```
at v=[Inf, -Inf, NaN]: native=[Inf, -Inf, NaN]  wasm=[Inf, -Inf, NaN]
```
