...
construct: "optimizer_unsound: `[0.0, 0.0, maximum(v)]` :: Vector{Float64}→Vector{Float64}"
...
# Gap `91c4fd42865b` — optimizer_unsound: `[0.0, 0.0, maximum(v)]` :: Vector{Float64}→Vector{Float64}
...
repro(v::Vector{Float64}) = [0.0, 0.0, maximum(v)]
_x = Float64[]
_c = deepcopy(_x)
_nat = try (:ok, repro(_c)); catch e; (:throw, e); end
_rt = Core.Compiler.widenconst(Base.code_typed(repro, (Vector{Float64},))[1][2])
_res = FuzzBridgeArgs.bridge_run_args(repro, (Vector{Float64},), [(deepcopy(_x),)]; rettype = _rt, opt=:size)[1]
...
## Diagnostic
```
at v=Float64[]: native=throw ArgumentError  wasm=[0, 0, 0]
```
