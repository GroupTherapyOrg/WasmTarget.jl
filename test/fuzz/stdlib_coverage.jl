...
const SPECS = StdSpec[
    ...
    StdSpec("Random", Random,
        ...
        "Seeded Xoshiro streams differentially fuzzed by test/fuzz/random_diff.jl: rand/randn/randexp (scalar), randperm/randcycle/shuffle & their `!`-variants, seed!, randsubseq/randsubseq!, randstring (ext overlay via native's OWN collection bulk fill). NB rand/randn are Base-owned (not in names(Random)) so they don't count below, but ARE verified. VERSION CAVEAT: randsubseq/randsubseq!/randstring are bit-exact on Julia ≤1.12 (counted here) but diverge wasm↔native on 1.13-rc1 (RNG-consumption changes — randsubseq is input-dependently fragile; randstring's charset bulk fill is no longer scalar/plain-Vector reproducible, same class as the SIMD fills); RANDOM_VERIFIED drops them on ≥1.13 → Random is 100% on ≤1.12, 73% on 1.13-rc1 (the 3 are documented boundary there, see FINDINGS — soundness-loop candidates). seed! is bit-exact on both. CAN'T: rand!/randn!/randexp! Float64-array fills route through an 8-lane SIMD bulk generator (llvmcall intrinsics; stream ≠ scalar for n≥8) — non-lowerable + not scalar-reproducible; bitrand (BitVector packed bits); default_rng/RandomDevice (host entropy); MersenneTwister state hits a codegen gap."),
]
