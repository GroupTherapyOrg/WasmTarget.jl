...
const SPECS = StdSpec[
    ...
    StdSpec("Random", Random,
        ...
        "Seeded Xoshiro streams differentially fuzzed by test/fuzz/random_diff.jl: the verifiable value+in-place surface — rand/randn/randexp (scalar), randperm/randcycle/shuffle & their `!`-variants, seed!, randsubseq, randstring (ext overlay that calls native's OWN collection bulk fill rand!(rng,v,chars), version-robust). NB rand/randn are Base-owned (not in names(Random)) so they don't count below, but ARE verified. randsubseq! is verified bit-exact on Julia ≤1.12 (counted here); on 1.13-rc1 it shows an input-dependent wasm↔native divergence (a found codegen gap, see FINDINGS) so RANDOM_VERIFIED drops it on ≥1.13 → Random is 91% there pending root-cause. CAN'T: rand!/randn!/randexp! Float64-array fills route through an 8-lane SIMD bulk generator (llvmcall intrinsics; stream ≠ scalar for n≥8) — non-lowerable + not scalar-reproducible; bitrand (BitVector packed bits); default_rng/RandomDevice (host entropy); MersenneTwister state hits a codegen gap."),
]
